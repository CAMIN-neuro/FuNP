...
echo -e "\n## Calculate Multi-Shell Multi-Tissue, Response function and Fiber Orientation Distribution"
# Response function
rf_wm="${tmpDir}/response_wm_dhollander.mif"
rf_gm="${tmpDir}/response_gm_dhollander.mif"
rf_csf="${tmpDir}/response_csf_dhollander.mif"
...
run_first_all -i ${tmpDir}/T1w_restore_brain -o ${tmpDir}/T1w -b
if [[ ! -f "${tmpDir}/T1w_all_fast_firstseg.nii.gz" ]]; then 
    Error "run_first_all failed, check the logs"
    exit;
else
    rm -rf ${tmpDir}/T1w.logs
fi

# Remove brainstem (label 16)
...
# first
rm -rf ${tmpDir}/T1.nii.gz
rm -rf ${tmpDir}/T1w*.bvars
rm -rf ${tmpDir}/T1w*.vtk
rm -rf ${tmpDir}/T1w.com*
rm -rf ${tmpDir}/T1w_all_fast_origsegs.nii.gz
rm -rf ${tmpDir}/T1w2b0.*
